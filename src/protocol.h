...
constexpr uint8_t HEARTBEAT_ACK  = 0x21;  // Fixed: was 0x11
constexpr uint8_t CMD_GPIO       = 0x30;
constexpr uint8_t CMD_PWM        = 0x31;
constexpr uint8_t CMD_DAC        = 0x32;
constexpr uint8_t CMD_CONFIG     = 0x33;

// Frame types (device → host)
constexpr uint8_t HANDSHAKE_RESP = 0x02;
constexpr uint8_t DATA_BATCH     = 0x10;
constexpr uint8_t HEARTBEAT      = 0x20;  // Fixed: was 0x12
constexpr uint8_t ACK            = 0x40;
constexpr uint8_t ERROR_FRAME    = 0x50;

// Config IDs for CMD_CONFIG
constexpr uint8_t CONFIG_ENABLE_CHANNEL = 0x02;
constexpr uint8_t CONFIG_RESET_DEVICE   = 0xFE;
constexpr uint8_t CONFIG_REQUEST_HANDSHAKE = 0xFF;

// Error codes for ERROR frame
constexpr uint8_t ERROR_BUFFER_OVERRUN   = 0x01;
constexpr uint8_t ERROR_ADC_ERROR        = 0x02;
constexpr uint8_t ERROR_INVALID_COMMAND  = 0x03;
constexpr uint8_t ERROR_HARDWARE_FAULT   = 0x04;
constexpr uint8_t ERROR_GENERIC          = 0xFF;

// ACK status codes
constexpr uint8_t ACK_SUCCESS        = 0x00;
constexpr uint8_t ACK_INVALID_PARAM  = 0x01;
constexpr uint8_t ACK_HARDWARE_ERROR = 0x02;
...
static const uint16_t DRAM_ATTR crc16_table[256] = {
...
// Helper to write uint16_t in little-endian
inline void writeUint16LE(uint8_t* dest, uint16_t value) {
    dest[0] = value & 0xFF;
    dest[1] = (value >> 8) & 0xFF;
}

// Helper to write uint32_t in little-endian
inline void writeUint32LE(uint8_t* dest, uint32_t value) {
    dest[0] = value & 0xFF;
    dest[1] = (value >> 8) & 0xFF;
...
inline uint16_t readUint16LE(const uint8_t* src) {
    return (uint16_t)src[0] | ((uint16_t)src[1] << 8);
}

// Helper to read uint32_t in little-endian
inline uint32_t readUint32LE(const uint8_t* src) {
